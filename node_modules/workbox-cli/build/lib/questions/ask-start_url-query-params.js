...
async function askQuestion() {
    return inquirer_1.prompt([{
            ...
            message: common_tags_1.oneLine `Does your web app manifest include search parameter(s)
...
async function askQueryParametersInStartUrl(defaultIgnoredSearchParameters = constants_1.constants.ignoreURLParametersMatching) {
    ...
    assert_1.default(ignoreURLParametersMatching.length > 0, errors_1.errors['no-search-parameters-supplied']);
    const ignoreSearchParameters = ignoreURLParametersMatching.trim().split(',').filter(Boolean);
    assert_1.default(ignoreSearchParameters.length > 0, errors_1.errors['no-search-parameters-supplied']);
    assert_1.default(ignoreSearchParameters.every(param => !param.match(/^[^\w|-]/g)), errors_1.errors['invalid-search-parameters-supplied']);
    return defaultIgnoredSearchParameters.concat(ignoreSearchParameters
