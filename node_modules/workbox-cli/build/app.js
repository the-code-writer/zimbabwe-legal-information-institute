...
async function runBuildCommand({ command, config, watch }) {
    ...
    logger_js_1.logger.log(`The service worker will precache ${count} URLs, ` +
        `totaling ${pretty_bytes_1.default(size)}.`);
    if (watch) {
...
const app = async (params) => {
    ...
    assert_1.default(params && Array.isArray(params.input), errors_js_1.errors['missing-input']);
    ...
    switch (command) {
        ...
        case 'copyLibraries': {
            assert_1.default(option, errors_js_1.errors['missing-dest-dir-param']);
        ...
        case 'injectManifest': {
            ...
            let configFromDisk;
            try {
                configFromDisk = read_config_js_1.readConfig(configPath);
            }
            catch (error) {
